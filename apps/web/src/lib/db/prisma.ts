        ...
        async $allOperations({ args, query, operation, model }) {
          ...
          if (WRITE_OPS.includes(operation)) {
            ...
            if (Array.isArray(data)) {
              a.data = data.map((d: Record<string, unknown>) => ({ ...d, tenantId }))
...
export async function getPrisma() {
  ...
  if (!tenantId) throw new Error('No tenant context — x-tenant-id header not set by middleware')
